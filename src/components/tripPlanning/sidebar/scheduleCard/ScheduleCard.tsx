...
export default function ScheduleCard() {
  ...
  const trip = useTripFunnelStore((s) => s.trip);

  const { deleteScheduleItem } = deleteTripItems();
  ...
  return (
    <>
      {schedule.map((place, idx) => {
        const dayPlan = daysPlan.find((d) => d.day === currentDay);
        const scheduleId = dayPlan?.id;
        const feedId = trip.boardId;
        const scheduleItemId = place.scheduleItemId;
